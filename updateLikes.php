...
if ($result->num_rows > 0) 
{
    ...
    if ($interaction['interaction_type'] == 'like') 
    {
        ...
        $deleteStmt->execute();
        $conn->query("UPDATE posts SET likes = likes - 1 WHERE postID = $postID");
    } 
    else 
    {
        $updateStmt = $conn->prepare("UPDATE post_interactions SET interaction_type = 'like' WHERE interaction_id = ?");
        $updateStmt->bind_param("i", $interaction['interaction_id']);
        $updateStmt->execute();
        $conn->query("UPDATE posts SET likes = likes + 1, dislikes = dislikes - 1 WHERE postID = $postID");
    }
} 
else 
{
    $insertStmt = $conn->prepare("INSERT INTO post_interactions (user_id, post_id, interaction_type) VALUES (?, ?, 'like')");
